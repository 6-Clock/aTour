...
def test_create_publish_list_spine(client, seeded_user):
    ...
    try:
        publish_response = client.patch(f"/api/posts/{post['post_id']}/publish")
...
def test_create_post_missing_title_rejected(client, seeded_user):
    response = client.post(
        "/api/posts", json={"booking_fee": "10.00", "max_group_size": 2}
    )
    assert response.status_code == 422


def test_create_post_negative_booking_fee_rejected(client, seeded_user):
    response = client.post(
        "/api/posts",
...
def test_create_post_zero_max_group_size_rejected(client, seeded_user):
    response = client.post(
        "/api/posts",
...
def test_create_post_boundary_values_accepted(client, seeded_user):
...
def test_publish_nonexistent_post_404(client, seeded_user):
    fake_id = "00000000-0000-0000-0000-000000000000"
    response = client.patch(f"/api/posts/{fake_id}/publish")
